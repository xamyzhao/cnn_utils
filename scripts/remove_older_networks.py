...
def remove_all_but_milestones_and_recent(in_dir, file_exts, num_prefix, milestone_interval, num_recent_to_keep=3,
                                         test=False):
	...
	if (len(nums) > num_recent_to_keep):
		...
		files_to_remove = [f for f in files if np.any(['{}{}_'.format(num_prefix, ntr) in os.path.basename(
...
if __name__ == '__main__':
	...
	ap.add_argument('-models_milestone', nargs='?', type=int, help='save any files with epochs % milestone == 0',
	                default=100)
	ap.add_argument('-figs_milestone', nargs='?', type=int, help='save any files with epochs % milestone == 0',
	                default=10)
	...
	ap.add_argument('-fig_num_prefix', nargs='?', type=str, help='number prefix e.g. epoch, batch, iter',
	                default='epoch')
	ap.add_argument('-test', action='store_true', help='turn on to see what will be removed without actually removing',
	                default=False)
	...
	while True:
		...
		if args.f:
			...
			for figs_folder in fig_dirs:
				remove_all_but_milestones_and_recent(in_dir=figs_folder, file_exts=fig_ext,

	...
	for i in range(n_convs):
		...
		if np.all(curr_shape == output_shape[:len(curr_shape)]):
			...
			if include_skips is not None and i < len(include_skips) and include_skips[i] is not None:
				...
				'''
				if not x_shape[-2] == skip_shape[-2]:
					tile_factor = [1] * len(x_shape)
					tile_factor[-2] = int(round(x_shape[-2] / skip_shape[-2]))
					skip = Lambda(lambda x:tf.tile(x, tile_factor))(include_skips[i])
				else:
					skip = include_skips[i]
				# now check if batch doesnt match, in case we are doing multiple samples
				x_shape = x.get_shape().as_list()
				skip_shape = skip.get_shape().as_list()
				'''
				if n_samples > 1:

...
def onehot_to_labels( oh, n_classes=0, label_mapping=None):
	...
	if type(label_mapping[0]) == str:
		labels_flat = np.empty( (np.prod(oh.shape[:-1]),), dtype='object')
		labels_flat[:] = ''
	else: 
		labels_flat = np.zeros( (np.prod(oh.shape[:-1]),), dtype=int )

	for i in range(oh_flat.shape[0]):
		argmax_idxs = np.where( oh_flat[i,:] == np.max(oh_flat[i,:]) )
		labels_flat[i] = label_mapping[argmax_idxs[0][0]]

	labels = np.reshape(labels_flat, oh.shape[:-1])
	'''
...
def onehot_to_top_k_labels( oh, k, n_classes=None, label_mapping=None):
	...
	if type(label_mapping[0]) == str:
		...
		labels_flat = np.zeros( (np.prod(oh.shape[:-1]),), dtype=int )
	for i in range(oh_flat.shape[0]):
		max_vals, max_idxs = zip(*sorted(zip(oh_flat[i,:].tolist(), np.linspace(0, oh_flat.shape[1], oh_flat.shape[1], endpoint=False, dtype=int).tolist())))
		argmax_idxs = max_idxs[:k]
		#argmax_idxs = np.where( oh_flat[i,:] == np.max(oh_flat[i,:]) )
#		if type(label_mapping[l]) == str:
#			labels_flat[ argmax_idxs ] = label_mapping[l]
#		else:
